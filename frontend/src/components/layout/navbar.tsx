...
import { Bell, Search, Command, Menu, Moon, Sun, Monitor } from "lucide-react";
  ...
  return (
    <header className="flex h-16 items-center justify-between border-b border-white/[0.06] bg-[var(--secondary,#0A0F1E)]/60 px-4 md:px-6 backdrop-blur-2xl">
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={openPalette}
          className="flex items-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-2 text-sm text-[#475569] transition-all hover:border-[#DC2626]/20 hover:bg-white/[0.05] hover:text-[#94A3B8]"
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="relative rounded-xl p-2.5 text-[#475569] transition-colors hover:bg-white/[0.06] hover:text-[#94A3B8]">
          ...
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-[#DC2626] shadow-sm shadow-[#DC2626]/50" />
        </button>

        <div className="flex items-center gap-2.5 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-1.5">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-[#DC2626] to-[#991B1B] text-[10px] font-bold text-white">
            {initials}
          </div>
